...
export function fetchGetUserRoles(userId: string) {
  return callRpc<Array<{ role_id: string; role_code: string; role_name: string }>>('get_user_roles', {
    p_user_id: userId
  })
}
...
export function fetchGetUserSessions(userId: string) {
  return callRpc<Array<{ id: string; created_at: string; expires_at: string; is_active: boolean }>>('get_user_sessions', {
    p_user_id: userId
  })
}
